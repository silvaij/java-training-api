...
import br.com.training.util.IsCpfValidation;
...
public class UserService {
    ...
    public User findByCpf(String cpf){
        String cpfv2 = IsCpfValidation.onlyNumbers(cpf);
    	Optional<User> obj = userRepository.findByCpf(cpfv2);
        return obj.orElseThrow(() -> new ResourceNotFoundException("CPF nao encontrado"));
    }

    @Transactional(readOnly = true)
    public User saveUser(UserDtoRequest dto){
           String str = IsCpfValidation.onlyNumbers(dto.getCpf());
    ...
    public User update(String cpf,User user){
        ...
        User userDb = findByCpf(IsCpfValidation.onlyNumbers(cpf));
